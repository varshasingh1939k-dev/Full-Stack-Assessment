...
export const Button = ({ children, variant = 'primary', size = 'md', loading = false, disabled = false, className = '', ...props }) => {
  ...
  const variants = {
    primary: 'bg-gradient-to-r from-blue-600 to-violet-600 hover:from-blue-500 hover:to-violet-500 text-white shadow-lg shadow-blue-900/20 hover:shadow-blue-500/30 border border-white/10 hover:-translate-y-0.5',
    ...
    link: 'text-blue-400 hover:text-blue-300 underline-offset-4 hover:underline shadow-none',
    ...
    <div className="relative">
      ...
      <input
        className={`block w-full text-sm bg-slate-950/80 border rounded-xl shadow-inner placeholder-slate-500 text-white transition-all duration-200
          ...
          ${error ? 'border-rose-500 focus:ring-rose-500/40' : 'border-white/10 hover:border-white/20 focus:ring-blue-500/40'}
...
const alertConfig = {
  ...
  info: { wrapper: 'bg-blue-500/10 border-blue-500/20 text-blue-300', icon: <Info className="w-4 h-4 text-blue-400 flex-shrink-0 mt-0.5" /> },
};
...
export const LoadingState = ({ label = 'Loading workspace...' }) => (
  <div className="flex flex-col items-center justify-center h-64 gap-4 animate-in">
    <div className="relative flex items-center justify-center w-12 h-12">
      <div className="absolute inset-0 rounded-xl border-t-2 border-blue-500 animate-spin"></div>
      <Loader2 className="w-6 h-6 text-violet-500 animate-spin" style={{ animationDirection: 'reverse', animationDuration: '1.5s' }} />
    </div>
...
export const Avatar = ({ name = '?', size = 'md', className = '' }) => {
  ...
  const colors = [
    'from-blue-600 to-violet-600', 'from-emerald-500 to-cyan-600', 
    'from-amber-500 to-orange-600', 'from-rose-500 to-red-600',
